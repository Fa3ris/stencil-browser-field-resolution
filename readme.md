...
`qs` requires `util` which is resolved to be the npm package `util` in the dev dependencies and not the 
built-in `util` from node. If it were the built-in `util` there is an error asking to use the plugin node polyfills.

But anyway, the `util` module should not be imported in the first place because `qs` has a browser field to exclude its util-inspect.js file
