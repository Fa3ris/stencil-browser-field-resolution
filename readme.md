Build succeeds but fails at runtime because of `process is not defined`

`object-inspect` requires `util` which is resolved to be the npm package `util` in the dev dependencies and not the 
built-in `util` from node. Without the package `util` present in the node_modules, there would be an error asking to use the plugin node polyfills.

But anyway, the `util` module should not be imported in the first place because `object-inspect` has a browser field to exclude its `util.inspect.js` file

It's probably due to the change in the rollup node-resolve plugin which has been moved to the `post` phase for its resolveId hook.
